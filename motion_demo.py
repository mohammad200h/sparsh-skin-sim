"""Run Leap+XELA grasp motions with live flex-sensor visualization."""
...
from util.motion_util import GRASP_PATTERNS, default_grasp_profile, grasp_target
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Leap+XELA grasp motion demo")
    parser.add_argument(
        "--grasp-pattern",
        choices=GRASP_PATTERNS,
        default="hold",
        help="Actuator motion to run (default: hold)",
    )
    parser.add_argument(
        "--visualize-force",
        action="store_true",
...
def main() -> None:
    ...
    grasp_profile = default_grasp_profile(args.grasp_pattern)
    data.ctrl[:] = grasp_target(model, float(data.time), grasp_profile)
    ...
    print(f"  grasp_pattern={grasp_profile.pattern}")
    print(f"  visualize_force={visualize_force}")
    print(
        "  Tip: use --grasp-pattern to select "
        "hold, pulse, squeeze, regrasp, tap, or shear."
    )
    ...
    try:
        with mujoco.viewer.launch_passive(model, data) as viewer:
            while viewer.is_running():
                ...
                data.ctrl[:] = grasp_target(
                    model, float(data.time), grasp_profile
                )
